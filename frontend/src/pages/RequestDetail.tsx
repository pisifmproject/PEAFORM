...
export default function RequestDetail() {
  ...
  useEffect(() => {
    fetchRequestData();
    const interval = setInterval(fetchRequestData, 3000); // Poll every 3s for real-time chat
    return () => clearInterval(interval);
  }, [id]);

  const prevQnaLengthRef = useRef(0);
  useEffect(() => {
    const qnaLength = data?.qna?.length || 0;
    if (messagesEndRef.current && qnaLength > prevQnaLengthRef.current) {
      messagesEndRef.current.scrollIntoView({ behavior: 'smooth' });
    }
    prevQnaLengthRef.current = qnaLength;
  }, [data?.qna?.length]);
