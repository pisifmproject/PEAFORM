...
const ROLES = ['admin', 'user', 'hod', 'hse', 'factory_manager', 'engineering_manager'];
...
export default function AdminPanel() {
  ...
  const stats = useMemo(() => {
    ...
    const approvers = users.filter(u => ['hod', 'hse', 'factory_manager', 'engineering_manager'].includes(u.role)).length;
  ...
  const handlePlantChange = async (userId: string, newPlant: string) => {
    try {
      ...
      setUsers(users.map(u => u.id === userId ? { ...u, plant: newPlant } : u));
    } catch (err: any) {
      alert(err.message);
    }
  };
    ...
    return (
      <Layout>
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="mt-2 text-slate-500 max-w-xs">You do not have administrative permissions to view this panel.</p>
        </div>
      </Layout>
    );
  }
      ...
      >
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1">Manage system users, roles, and global configurations.</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <span className="text-sm font-bold text-slate-500">Loading user data...</span>
            </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-200">
                    <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">User Information</th>
                    <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">NIK</th>
                    <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Role</th>
                    <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Plant Assignment</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredUsers.map((u) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span className="text-sm font-mono text-slate-600">{u.nik}</span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        {['hod', 'hse', 'factory_manager'].includes(u.role) ? (
                          <div className="relative min-w-[140px]">
                            <select
                              value={u.plant || ''}
                              onChange={(e) => handlePlantChange(u.id, e.target.value)}
                          ...
                          <span className="text-xs text-slate-400 italic font-medium">Not Applicable</span>
                        )}
                      </td>
                      <td className="px-6 py-4 text-right">
                        {u.id !== user.id ? (
                          ...
                          <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-md">YOU</span>
                        )}
                      </td>
                    </motion.tr>
                  ))}
...
function AdminStatCard({ label, value, icon, color }: { label: string, value: number, icon: React.ReactNode, color: string }) {
  return (
    <motion.div 
      variants={{
